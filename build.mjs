...
await mkdir(new URL("./icons/", assets), { recursive: true });
...
for (const file of ["index.html", "styles.css", "app.js", "sw.js", "manifest.webmanifest"]) {
  await copyFile(new URL(`./${file}`, import.meta.url), new URL(`./${file}`, assets));
}

await cp(new URL("./icons/", import.meta.url), new URL("./icons/", assets), { recursive: true });
